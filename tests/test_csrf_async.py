...
async def test_csrf_off():
    ...
    @csrf_OFF.post("/post")
    async def post_off(request) -> Annotated[Response[SuccessResult], 200]:
...
async def test_csrf_on():
    ...
    @csrf_ON.post("/post")
    async def post_on(request) -> Annotated[Response[SuccessResult], 200]:
        return Response(200, {"success": True})
...
async def test_csrf_exempt_async():
    ...
    async def post_on_with_exempt(request) -> Annotated[Response[SuccessResult], 200]:
        return Response(200, {"success": True})
