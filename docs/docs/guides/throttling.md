...
```Python
from hattori.throttling import AnonRateThrottle, AuthRateThrottle
...
```Python
from hattori.throttling import UserRateThrottle
...
Will throttle by Django Hattori [authentication](authentication.md) to a given rate of requests across the API.  Unauthenticated requests will fall back to using the IP address of the incoming request to generate a unique key to throttle against.
...
```Python
from hattori.throttling import AnonRateThrottle
